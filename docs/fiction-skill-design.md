...
1. Standard／Longform 先討論大綱，未經確認不准寫正文；規格完整的 Quick 任務可直接執行。
2. 寫作前先讀取/重建故事狀態，避免忘記前面劇情。
3. 大綱階段檢查衝突壓力，避免故事太和平。
4. 高智商角色不能靠旁白作弊，要靠可回放的觀察、推理、代價與限制。
5. 謀略不能完美，要在現有爛資源下做最佳化。
6. 使用者要求或文本出現明顯症狀時才去除 AI 味，而且不能把作者風格洗掉。

一句話版本：

```text
標準／長篇模式未經大綱確認，不准動筆；規格完整的 Quick 任務可把使用者提示視為已核准 brief。
缺少連續性資料，不准硬寫。
場景沒有衝突壓力或明確戲劇功能，不准進正文。
高智商不能等於作者作弊。
完美計畫太假，必須是有限資源下的最佳解。
```
...
使用者提出故事想法
↓
選擇 Quick／Standard／Longform
↓
Quick：規格完整時把提示視為 approved brief，只有重大選擇未決才先列大綱
Standard／Longform：整理或詢問後產出可討論版大綱，等待確認後才寫正文
↓
正文完成後只跑與本次問題、症狀或使用者要求相關的檢查：
1. 劇情功能檢查
2. POV / 角色知識檢查
3. 衝突壓力檢查
4. 對話自然度檢查
...
In standard and longform modes, never draft original prose before the required outline is approved. Quick mode may treat a sufficiently specific request as the approved brief.
...
Beats are weak only when they lack desire, obstacle, cost, turn, and an explicitly stated clear dramatic function. Contrast, intimacy, recovery, reflection, setup, consequence, or deliberate stillness may carry a beat without manufactured pressure.
```
...
A peaceful scene is valid when it creates contrast, dread, intimacy, recovery, reflection, setup, or another clear dramatic function.
```
...
Run an anti-AI-flavour pass only when explicitly requested or when the prose shows plastic, templated symptoms. Preserve intentional roughness, ambiguity, rhythm, and authorial voice.
...
目前已升級為 Codex Plugin 架構，以 `sheep-story` 作為根目錄 plugin：
...
6. 標準／長篇模式不准未經大綱確認就動筆；Quick 模式只放行規格完整的有限任務。
